...
Restart the proxy after changing this block. `/vauth reload` reloads the operator-facing core
settings, but a full restart is the supported way to apply TOTP service settings and to clear
all pending enrollment or login challenges predictably.
...
player who has a saved token. To permanently remove a token, use
`/vauth 2fa-remove <nickname>` for each verified recovery request. Back up the database first;
avoid bulk SQL resets unless you have a reviewed, database-specific maintenance plan.
...
If a code generated by an external compliant authenticator app fails to verify, file an issue
with sanitized details: VeloAuth/Velocity/Java versions, authenticator application name,
approximate clock drift, database type and relevant redacted logs. **Never include a TOTP secret,
a live or expired code, a complete `otpauth://` URI, database dump or player credentials.**
